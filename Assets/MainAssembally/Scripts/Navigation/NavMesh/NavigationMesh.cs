...
public class NavigationMesh : MonoBehaviour
{
    ...
    public IEnumerable<NavMeshCell> GetCellsInBounds(Collider collider, bool createNewCell = true)
    {
        if(collider is MeshCollider meshCollider)
        {
            ...
            bounds = new Bounds(collider.transform.localToWorldMatrix.MultiplyPoint(bounds.center), Vector3.Scale(collider.transform.lossyScale, bounds.size));
            return GetCellsInBounds(bounds, collider.transform.rotation, createNewCell);
        }
        else if(collider is BoxCollider boxCollider)
        {
            Bounds bounds = new Bounds(collider.transform.localToWorldMatrix.MultiplyPoint(boxCollider.center), Vector3.Scale(collider.transform.lossyScale, boxCollider.size));
            return GetCellsInBounds(bounds, collider.transform.rotation, createNewCell);
        }
        else if(collider is SphereCollider sphereCollider)
        {
            Bounds bounds = new Bounds(collider.transform.localToWorldMatrix.MultiplyPoint(sphereCollider.center), Vector3.Scale(collider.transform.localScale, Vector3.one * sphereCollider.radius * 2f));
            return GetCellsInBounds(bounds, collider.transform.rotation, createNewCell);
        }
        else
        {
